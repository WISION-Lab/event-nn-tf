...
video_raw = read_video(path.join("inputs", "wave"))[:201]
...
save_video(visualized, path.join("outputs", "wave_openpose_event.mp4"), keep_images=True)

pafs, heatmaps = model.predict_conventional(video, temporal_axis=1, static_graph=True)
output = postprocess_video(video_raw, pafs[0], heatmaps[0], mode="mpii")
visualized = visualize_video(video_raw, output)
save_video(visualized, path.join("outputs", "wave_openpose_conventional.mp4"), keep_images=True)
...
with open(path.join("operations", "examples", "wave_openpose_event.p"), "wb") as f:
    pickle.dump(ops, f)

ops = model.count_ops_conventional(video, temporal_axis=1, temporal_mean=False, static_graph=True)
print("Conventional ops:")
print_dict(reduce_ops_all(filter_wrapped_ops(ops, model)))
with open(path.join("operations", "examples", "wave_openpose_conventional.p"), "wb") as f:
    pickle.dump(ops, f)
