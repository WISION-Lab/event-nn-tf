...
# Load the data.
(frames, flow, _), n_items = sintel.load_data(path.join("data", "sintel"), "train", size=size)
...
with open(path.join("operations", "evaluate", "sintel_train_conventional.p"), "wb") as f:
    pickle.dump(ops, f)
...
print("Event ops:")
print_dict(reduce_ops_all(filter_wrapped_ops(ops, model)))
with open(path.join("operations", "evaluate", "sintel_train_event.p"), "wb") as f:
    pickle.dump(ops, f)
