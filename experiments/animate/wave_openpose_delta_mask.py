...
with open(path.join("outputs", "wave_openpose_delta_mask.p"), "rb") as f:
...
base_dir = path.join("animations", "wave_openpose_delta_mask")
...
for deltas, masks, gate_name, index in zip(outputs[:n], outputs[n:], GATE_NAMES, indices):
    if gate_name not in output_filter:
        continue
    save_video(
        delta_cmap(deltas[..., index], scale=4.0),
        path.join(base_dir, "deltas_{}.mp4".format(gate_name)),
        resize_method="nearest",
    )
    save_video(
        mask_cmap(masks[..., index], scale=4.0),
